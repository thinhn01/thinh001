...
module.exports.run = async function ({ api, event, args, Users }) {
  if ((this.config.credits) != "D-Jukie mod by vloc") { return api.sendMessage(`🕉️Phát hiện credits đã bị thay đổi`, event.threadID, event.messageID)}
  ...
  let getWanted = (
    await axios.get(encodeURI(`https://i.imgur.com/WneeBIU.jpg`), {
      responseType: "arraybuffer",
    })
  ).data;
...
/*---------------------------------------------*/
  ...
  ctx.drawImage(baseAva, 10, 110, 175, 175);
  ...
  ctx.textAlign = "start";
  fontSize = 20;
  ctx.fillText(`» Số thành viên: ${threadMem}`, 210, 150);
  ctx.fillText(`» Quản trị viên: ${qtv}`, 210, 190);
  ctx.fillText(`» Nam: ${nam}`, 210, 230);
  ctx.fillText(`» Nữ: ${nu}`, 210, 270);
  ctx.fillText(`» Tổng số tin nhắn: ${sl}`, 210, 310);
  ctx.font = `${fontsOthers}px Play-Bold`;
  ctx.fillStyle = "#ffffff";
  ctx.textAlign = "start";
  fontSize = 20;
  ctx.fillText(`➤ ID BOX: ${id}`, 150, 405);
  ctx.font = `${fontsOthers}px Play-Bold`;
  ctx.fillStyle = "#ffffff";
  ctx.textAlign = "start";
  fontSize = 20;
  ctx.fillText(`➤ Cùng ${parseInt(threadMem)-3} thành viên khác...`, 150, 380);
  ctx.beginPath();
