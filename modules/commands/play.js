...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  try {   
    await ytdl(handleReply.link[event.body - 1],{ filter: 'audioonly'})
      ...
      .on("close", () => {
        if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`).size > 104000000) return api.sendMessage('Không thể gửi file vì dung lượng lớn hơn 100MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`), event.messageID);
  ...
  if (urlValid) {
    try { 
      ...
      ytdl(args[0])
        ...
        .on("close", () => {
          if (statSync(__dirname + `/cache/${id}.m4a`).size > 104000000) return api.sendMessage('Bài Này Dài Wa Không Gửi Được , Đổi Bài !', event.threadID, () => unlinkSync(__dirname + `/cache/${id}.m4a`), event.messageID);
          else{
