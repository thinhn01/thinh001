...
module.exports.onLoad = async () => {
    ...
    await require('axios').get("https://raw.githubusercontent.com/Shiron-Official/mirai-modules/main/version.json").then(res => {
        if (res.data["baucua_x022"] != this.config.version) console.log("-BAUCUA ĐÃ CÓ PHIÊN BẢN MỚI, LIÊN HỆ ShironUwU ĐỂ ĐƯỢC CẬP NHẬT-");
    })
    let path = __dirname + '/baucua/';
    if (!fs.existsSync(path)) fs.mkdirSync(path, { recursive: true });
    await require("axios").get("https://raw.githubusercontent.com/Shiron-Official/base64_baucua/main/data_baucua.json").then(async (res) => {
