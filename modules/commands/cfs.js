...
const { threadID, messageID, senderID } = event;
	switch (args[0]) {
		...
		case 'find': {
			...
			var res = (await axios.get(`https://api.mincute.repl.co/cfsdata`)).data
				res.reverse()
...
const { threadID, messageID, senderID, body } = event;
	switch (handleReply.type) {
		case 'send': {
			...
			var res = await axios.get(`https://api.mincute.repl.co/cfs?message=${encodeURI(body)}&senderID=${senderID}&name=${encodeURI((await Users.getData(senderID)).name)}`);
...
const { threadID, messageID, senderID } = event;
	switch (handleReaction.type) {
		case 'react': {
			var res = await axios.get('https://api.mincute.repl.co/react?index=' + handleReaction.index);
		}
	}
}