...
let text = args.join(" ")
  if (!text) return api.sendMessage('Vui lòng nhập link tikvideo', event.threadID, event.messageID);
  const length_0 = parseInt(text.length)
 const link = args.join(" ").trim().replace(/\s+/g, " ").replace(/(\s+\|)/g, "|").replace(/\|\s+/g, "|").split("|")[0];
const res = await axios.get
(`https://hoangdogianguyenofficial.herokuapp.com/tiktok?link=${link}`);
