...
module.exports.onLoad = async () => {
  ...
  const path = resolve(__dirname, 'cache/canvas', 'etou.png');
...
async function makeImage({ one, two }) {
  ...
  let etou_img = await jimp.read(__root + "/etou.png");
  let pathImg = __root + `/etou_${one}_${two}.png`;
  ...
  etou_img.composite(circleOne.resize(130, 130), 190, 155).composite(circleTwo.resize(150, 150), 525, 45);

  let raw = await etou_img.getBufferAsync("image/png");
...
module.exports.run = async function({ event, api, args, Users }) {
  ...
  if (!mention && !event.messageReply) return api.sendMessage("Vui lòng tag hoặc reply(Phản hồi) tin nhắn của 1 người", threadID, messageID);
    ...
    if (args.join(" ").length == 0) var text = 'Etou 😳👉👈';
