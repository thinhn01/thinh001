...
async function makeImage({ one, two }) {
  ...
  let marry_img = await jimp.read(__root + "/marry.png");
  ...
  marry_img.composite(circleOne.resize(60, 60), 200, 23).composite(circleTwo.resize(60, 60), 136, 40);

  let raw = await marry_img.getBufferAsync("image/png");
...
module.exports.run = async function({ event, api, args, Users }) {
  ...
  if (!mention && !event.messageReply) return api.sendMessage("Vui lòng tag hoặc reply(Phản hồi) tin nhắn của 1 người", threadID, messageID);
    ...
    if (args.join(" ").length == 0) var text = 'Cơm chó ngon lắm :0. Chúc hai bạn hạnh fúck 😡';
