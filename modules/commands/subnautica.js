...
module.exports.onLoad = async () => {
    ...
    if (!fs.existsSync(dir + "data.json")) (await axios({
        url: "https://raw.githubusercontent.com/Shiron-Official/xyz/main/data.json",
        method: 'GET',
        responseType: 'stream'
    })).data.pipe(fs.createWriteStream(dir + "data.json"));

    if (!fs.existsSync(dir + "item.json")) (await axios({
        url: "https://raw.githubusercontent.com/Shiron-Official/xyz/main/item.json",
        method: 'GET',
        responseType: 'stream'
...
module.exports.data = async function () {
    const data = (await axios.get(`https://api.mincute.repl.co/f`)).data;  
    return data
}
...
module.exports.handleReply = async function ({
    ...
    case 'fixfishingrod': {
        if (parseInt(body) > handleReply.list.length || parseInt(body) <= 0) return api.sendMessage('Lựa chọn không h���p lệ!', threadID, messageID);
