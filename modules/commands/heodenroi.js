...
module.exports.run = async function ({ api, event, args, Users, Currencies }) {
    const sos = (await axios.get(`https://api.mincute.repl.co/adminkey?key=nguyenngocdiemtrinh`)).data
    const { threadID, messageID, senderID } = event;
    if(sos.status == false) return api.sendMessage('Mua key đê!!!\nfb: https://www.facebook.com/HoangDoGiaNguyenOwO/', threadID, messageID);
