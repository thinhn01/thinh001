...
async function makeImage({ one, two }) {
  ...
  let damdit_img = await jimp.read(__root + "/damdit.png");
  ...
  damdit_img.composite(circleOne.resize(180, 180), 264, 220).composite(circleTwo.resize(180, 180), 264, 220);

  let raw = await damdit_img.getBufferAsync("image/png");
...
module.exports.run = async function({ event, api, args, Users }) {
  ...
  if (!mention && !event.messageReply) return api.sendMessage("Vui lòng tag hoặc reply(Phản hồi) tin nhắn của 1 người", threadID, messageID);
    ...
    if (args.join(" ").length == 0) var text = 'Sướng quá ah ah ah...Đã bị hấp diêm 😟😟';
