...
module.exports.languages = {
    "vi": {
        ...
        "moneyBetNotEnough": "[ XIDACH ] Số tiền bạn đặt lớn hơn số dư của bạn!",
...
module.exports.onLoad = async () => {
    const fs = require("fs");
    await require('axios').get("https://raw.githubusercontent.com/Shiron-Official/mirai-modules/main/version.json").then(res => {
        if (res.data["xidach_x034"] != this.config.version) console.log("-XIDACH ĐÃ CÓ PHIÊN BẢN MỚI, LIÊN HỆ ShironUwU ĐỂ ĐƯỢC CẬP NHẬT-");
    })
    let path = __dirname + '/poker/';
    if (!fs.existsSync(path)) fs.mkdirSync(path, { recursive: true });
    await require("axios").get("https://raw.githubusercontent.com/Shiron-Official/base64_poker/main/data.json").then(async (res) => {
...
module.exports.run = async function({ api, event, args, getText, Users, Currencies }) {
    ...
    switch (args[0]) {
        ...
        case "leave":
            if (threadID in global.client.xidach_otm) {
                if (!global.client.xidach_otm[threadID].data.find(p => p.id == senderID)) return send(getText("notJoined"));
