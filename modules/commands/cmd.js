module.exports.config = {
    ...
    hasPermssion: 2,
    credits: "Mirai Team",
    description: "Quản lý/Kiểm soát toàn bộ lệnh của bot",
    commandCategory: "Admin",
    usages: "[load/unload/loadAll/unloadAll/info] [tên lệnh]",
    cooldowns: 5,
    dependencies: {
        "fs-extra": "",
        "child_process": "",
    ...
    for (const nameModule of moduleList) {
        try {
            ...
            if (!command.config || !command.run || !command.config.commandCategory) 
                throw new Error('Lệnh không đúng định dạng!');
            ...
            if (command.config.envConfig && typeof command.config.envConfig == 'Object') try {
                ...
                throw new Error('Không thể tải config lệnh, lỗi: ' + JSON.stringify(error));
            }
            if (command['onLoad']) try {
                ...
                throw new Error('Không thể onLoad lệnh, lỗi: ' + JSON.stringify(error), 'error');
            }
...
const unloadModule = function ({ moduleList, threadID, messageID }) {
    ...
    return api.sendMessage(`Đã hủy tải thành công ${moduleList.length} lệnh ✓`, threadID, messageID);
}
...
module.exports.run = function ({ event, args, api }) {
    ...
    switch (args[0]) {
        case "load": {
            if (moduleList.length == 0) return api.sendMessage("Tên lệnh không được để trống!", threadID, messageID);
        ...
        case "info": {
            ...
            if (!command) return api.sendMessage("Lệnh bạn nhập không tồn tại!", threadID, messageID);
