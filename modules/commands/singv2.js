...
module.exports.handleReply = async function ({
    ...
    try {
        const result = await axios.get(`https://hoangdogianguyenofficial.herokuapp.com/sing?link=https://www.youtube.com/watch?v=${handleReply.link[event.body - 1]}`);
...
module.exports.run = async function ({
    ...
    if (args.join(" ").indexOf("https://") == 0) {
        ...
        const result = await axios.get(`https://hoangdogianguyenofficial.herokuapp.com/sing?link=https://www.youtube.com/watch?v=${linkUrlSing}`);
        path1 = __dirname + `/cache/${event.senderID}.m4a`
        const getms = (await axios.get(`${result.data.link}`, {
            responseType: "arraybuffer"
        })).data;
        fs.writeFileSync(path1, Buffer.from(getms, "utf-8"));
        if (fs.statSync(__dirname + `/cache/${event.senderID}.m4a`).size > 104000000) return api.sendMessage('Không thể gửi file vì dung lượng lớn hơn 25MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${event.senderID}.m4a`), event.messageID);
        else return api.sendMessage({
            body: `» ${result.data.title}`,
            attachment: fs.createReadStream(__dirname + `/cache/${event.senderID}.m4a`)
        }, event.threadID, () => fs.unlinkSync(__dirname + `/cache/${event.senderID}.m4a`), event.messageID)
