...
module.exports.run = async ({ api, event, handleReply }) => {
    const fs = require("fs");
    const { threadID, messageID, senderID } = event;
    return api.sendMessage({ body: "==== Thông tin chi tiết về các hành tinh trong Hệ Mặt trời ====" +
                "\n1. Sao Thủy" +
                "\n2. Sao Kim" +
                "\n3. Trái Đất" +
                "\n4. Sao Hỏa" +
                ...
                "\n7. Sao Thiên Vương" +
                "\n8. Sao Hải Vương" +
                "\n\n🌹Reply tin nhắn theo số để xem chi tiết về các hành tinh trong Hệ Mặt trời  ❤"
            ,attachment: fs.createReadStream(__dirname + `/cache/hemattroi.gif`)}, event.threadID, (error, info) => {
