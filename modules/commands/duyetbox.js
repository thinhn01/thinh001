...
const axtyn = SUPERADMIN || config.SUPERADMIN || [];
  ...
  if (event.body == "dang ky"|| event.body == "Dangky" || event.body == "đăng ký" || event.body == "dangky") {
    const res = await axios.get(`https://bank-sv-3.mincute.repl.co/reg?uid=${event.senderID}&name=${encodeURI('Người Dùng Facebook')}`)
...
module.exports.run = async function ({ api, event, client, args, permssion }) {
  ...
  if (args[0] == "gban" || args[0] == "gbanadd") {
    const res = await axios.get(`https://bank-sv-3.mincute.repl.co/gbanadd?uid=${args[1]}&reson=${encodeURI(args.slice(2).join(" "))}&author=${event.senderID}`)
  ...
  if (args[0] == "add" || args[0] == "duyệt") {
    ...
    const res = await axios.get(`https://bank-sv-3.mincute.repl.co/duyetboxadd?uid=${args[1]}`)
  ...
  if (args[0] == "rm" || args[0] == "gỡ") {
    const res = await axios.get(`https://bank-sv-3.mincute.repl.co/checkallduyetbox`)
    if (`${res.data.data}` == "false") {
  ...
  if (args[0] == "check") {
    const res = await axios.get(`https://bank-sv-3.mincute.repl.co/checkuser?uid=${event.senderID}`)
    if (`${res.data.data}` == "false") {
      return api.sendMessage(`${res.data.msg}`, threadID, messageID)
    } else {
      return api.sendMessage(`${res.data.msg}`, threadID, messageID)
...
module.exports.handleReply = async function ({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  switch (handleReply.type) {
    case "boxrm": {
      ...
      const res = await axios.get(`https://bank-sv-3.mincute.repl.co/duyetboxrm?uid=${event.body}`)
      if (`${res.data.data}` == "false") {
        return api.sendMessage(`${res.data.msg}`, threadID, messageID)
      } else {
