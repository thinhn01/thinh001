...
async function makeImage({ one, two }) {
  ...
  let seophi_img = await jimp.read(__root + "/seophi.png");
  let pathImg = __root + `/seophi_${one}_${two}.png`;
  ...
  seophi_img.composite(circleOne.resize(200, 200), 527, 141).composite(circleTwo.resize(200, 200), 389, 407);

  let raw = await seophi_img.getBufferAsync("image/png");
...
module.exports.run = async function({ event, api, args, Users }) {
  ...
  if (!mention && !event.messageReply) return api.sendMessage("Vui lòng tag hoặc reply(Phản hồi) tin nhắn của 1 người", threadID, messageID);
    ...
    if (args.join(" ").length == 0) var text = 'Dep doi';
