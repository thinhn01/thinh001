...
module.exports.onLoad = async () => {
  ...
  const path = resolve(__dirname, 'cache/canvas', 'tamcung.png');
...
async function makeImage({ one, two }) {
  ...
  let tamcung_img = await jimp.read(__root + "/tamcung.png");
  let pathImg = __root + `/tamcung_${one}_${two}.png`;
  ...
  tamcung_img.composite(circleOne.resize(140, 140), 247, 243 ).composite(circleTwo.resize(150, 150), 382, 231);

  let raw = await tamcung_img.getBufferAsync("image/png");
...
module.exports.run = async function({ event, api, args, Users }) {
  ...
  if (!mention && !event.messageReply) return api.sendMessage("Vui lòng tag hoặc reply(Phản hồi) tin nhắn của 1 người", threadID, messageID);
    ...
    if (args.join(" ").length == 0) var text = 'Bạn rất may mắn😄😄,Khi được tắm cùng tôi 😝😝';
