...
/*---------------------------------------------*/
  ...
  ctx.drawImage(baseAvata, 50, 130, 270, 270);
    ...
    var birthday = res.data.data.birthday ? `${res.data.data.birthday}` : "Đã Ẩn Thông Tin";
    var love = res.data.data.relationship_status ? `${res.data.data.relationship_status}` : "Độc Thân"
    var location = res.data.data.location ? `${res.data.data.location}` : "Đã Ẩn Thông Tin"
  ...
  ctx.textAlign = "start";
  fontSize = 22;
  ctx.fillText(`Full Name : ${res.data.data.fullname}`, 410, 172);
...
ctx.fillStyle = "#FFFFE0";
  ctx.textAlign = "start";
  fontSize = 22;
  ctx.fillText(`Lượt Follow: ${res.data.data.follow_user} người theo dõi`, 410, 244);
...
ctx.fillStyle = "#EEC591";
  ...
  ctx.textAlign = "start";
  fontSize = 23;  
  ctx.fillText(`Link Facebook: ${res.data.data.url_profile}`, 30, 450);
  ctx.beginPath();
