...
module.exports.config = {
	name: "loccacanh", // Tên lệnh, được sử dụng trong việc gọi lệnh
...
module.exports.run = async ({ api, event, args, Threads }) => {
	...
	if(!args[0]) return api.sendMessage(" 『𝐃𝐄𝐄𝐏』Bot đang tiến hành lọc... ", threadID);
	...
	if(!(adminIDs.map(e => e.id).some(e => e == api.getCurrentUserID()))) return api.sendMessage("『𝐃𝐄𝐄𝐏』bạn cần phải cho bot làm quản trị viên", threadID);
	let Array = [];
