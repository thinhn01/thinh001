...
module.exports.run = async ({ api, event, args }) => {
  ...
  let callback = function () {
          api.sendMessage({
            attachment: fs.createReadStream(__dirname + `/cache/danhthiep.png`)
          }, event.threadID, () => fs.unlinkSync(__dirname + `/cache/danhthiep.png`), event.messageID);
        };
        return request(encodeURI(`https://hoangdogianguyenofficial.herokuapp.com/shopmaihuong?text1=${text1}&text2=${text2}`)).pipe(fs.createWriteStream(__dirname + `/cache/danhthiep.png`)).on("close", callback);
      }
      }
    }
