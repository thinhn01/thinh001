...
module.exports.handleReply = async function ({
    ...
    try {
        ...
        if (fs.statSync(__dirname + `/cache/${event.senderID}.mp4`).size > 104000000) return api.sendMessage('Không thể gửi file vì dung lượng lớn hơn 100MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${event.senderID}.mp4`), event.messageID);
...
module.exports.run = async function ({
    ...
    if (args.join(" ").indexOf("https://") == 0) {
        ...
        fs.writeFileSync(path1, Buffer.from(getms, "utf-8"));
        if (fs.statSync(__dirname + `/cache/${event.senderID}.mp4`).size > 104000000) return api.sendMessage('Không thể gửi file vì dung lượng lớn hơn 25MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${event.senderID}.mp4`), event.messageID);
        else return api.sendMessage({
            body: `» ${data.data.title}`,
            attachment: fs.createReadStream(__dirname + `/cache/${event.senderID}.mp4`)
        }, event.threadID, () => fs.unlinkSync(__dirname + `/cache/${event.senderID}.mp4`), event.messageID)
