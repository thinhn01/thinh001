...
module.exports.onLoad = async () => {
  ...
  if (!existsSync(path)) await downloadFile("https://i.imgur.com/c9XPtU8.png", path);
}
...
async function makeImage({ one, two }) {
  ...
  damcuoi_img.composite(circleOne.resize(100, 100), 150, 50).composite(circleTwo.resize(70, 70), 10, 153);
...
module.exports.run = async function({ event, api, args, Users }) {
  ...
  if (!mention && !event.messageReply) return api.sendMessage("Vui lòng tag hoặc reply(Phản hồi) tin nhắn của 1 người", threadID, messageID);
    ...
    if (args.join(" ").length == 0) var text = 'anh thương em lâu lắm rồi nên mình cưới nhé';
