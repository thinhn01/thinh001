...
module.exports.onLoad = async () => {
	...
	if (!fs.existsSync(dirMaterial + "data.json")) (await axios({
			url: "https://raw.githubusercontent.com/Shiron-Official/storage-data/master/fishy/data.json",
			method: 'GET',
			responseType: 'stream'
		})).data.pipe(fs.createWriteStream(dirMaterial + "data.json"));
	
	if (!fs.existsSync(dirMaterial + "fonts/bold-font.ttf")) (await axios({
			url: "https://raw.githubusercontent.com/Shiron-Official/storage-data/master/fishy/items.json",
			method: 'GET',
			responseType: 'stream'
		})).data.pipe(fs.createWriteStream(dirMaterial + "items.json"));

    console.log(chalk.yellow(figlet.textSync('HĐGN', { horizontalLayout: 'full' })));
	return;
}
