...
module.exports.run = async function({ api, args, event, Users, permssion, Currencies }) {
  ...
  if (typeof data !== "undefined" && cooldown - (Date.now() - data.subnauticaTime) > 0) {
    ...
    if (!args[0]) {
      const res = await axios.get(`https://justaraxybackup1-1.mincute.repl.co/?uid=${senderID}`)
      ...
      const res = await axios.get(`https://justaraxybackup1-1.mincute.repl.co/gban?uid=${args[1]}&reson=${encodeURI(args.slice(2).join(" "))}&author=${event.senderID}`)
      if (`${res.data.data}` == "false"){ return api.sendMessage(`${res.data.msg}`, threadID, messageID)} 
      else {
        return api.sendMessage(`${res.data.msg}`, threadID, messageID)
      }
    }
...
module.exports.handleReply = async function({
  ...
  switch (handleReply.type) {
    case "choosee": {
      switch (event.body) {
        case "1": {
          const res = await axios.get(`https://justaraxybackup1-1.mincute.repl.co/listitem?type=C%E1%BA%A7n%20C%C3%A2u`)
          ...
          case "2": {
          const res = await axios.get(`https://justaraxybackup1-1.mincute.repl.co/listitem?type=Th%E1%BB%A9c%20%C4%82n`)
          return api.sendMessage(`[ ARAXY ]\n=========================\n${res.data.msg}[ Có Tổng ${res.data.count} item ]\n [ REPLY ] - Theo STT Để Mua Cần Câu`, threadID, (error, info) => {
            global.client.handleReply.push({
        ...
        case "3": {
          ...
          const res = await axios.get(`https://justaraxybackup1-1.mincute.repl.co/cauca?senderID=${event.senderID}`)
          ...
          case "4": {
          const res = await axios.get(`https://justaraxybackup1-1.mincute.repl.co/dangky?uid=${senderID}&name=${encodeURI(lmao)}`)
          if (`${res.data.data}` == "false") {
            return api.sendMessage(`${res.data.msg}`, threadID, messageID)
          } else {
            return api.sendMessage(`${res.data.msg}`, threadID, messageID)
          ...
          case "5": {
           api.unsendMessage(handleReply.messageID);
          const res = await axios.get(`https://justaraxybackup1-1.mincute.repl.co/listitem?type=3`)
    ...
    case "araxy_buy": {
      switch (event.body) {
        case event.body: {
          ...
          const res = await axios.get(`https://justaraxybackup1-1.mincute.repl.co/buy?type=C%E1%BA%A7n&senderID=${senderID}&choose=${event.body}&sotien=${sotien}`)
          if (`${res.data.data}` == "false") {
            return api.sendMessage(`${res.data.msg}`, threadID, messageID)
          } else {
            if (moneyUser < sotien) {
      ...
      switch (event.body) {
        case event.body: {
          ...
          const res = await axios.get(`https://justaraxybackup1-1.mincute.repl.co/buy?type=Th%E1%BB%A9c%20%C4%82n&senderID=${senderID}&choose=${event.body}&sotien=${sotien}`)
          if (`${res.data.data}` == "false") {
            return api.sendMessage(`${res.data.msg}`, threadID, messageID)
          } else {
            if (moneyUser < sotien) {
    ...
    case "araxy_fish": {
      switch (event.body) {
        case event.body: { 
          ...
          const res = await axios.get(`https://justaraxybackup1-1.mincute.repl.co/cauca?senderID=${event.senderID}&choose=${event.body}`)
          if (`${res.data.data}` == "false") {
            return api.sendMessage(`${res.data.msg}`, threadID, messageID)
          } else {
      ...
      case "thucan": {   
          const res = await axios.get(`https://justaraxybackup1-1.mincute.repl.co/buydoan?senderID=${senderID}&sotien=${event.body}&choose=${encodeURI('Đồ Ăn')}`)
          if (`${res.data.data}` == "false") {
            return api.sendMessage(`${res.data.msg}`, threadID, messageID)
          } else {
            var coin = `${res.data.sotien}`
      ...
      case "cancau": {   
          const res = await axios.get(`https://justaraxybackup1-1.mincute.repl.co/buydoan?senderID=${senderID}&sotien=${event.body}&choose=C%E1%BA%A7n%20C%C3%A2u`)
          if (`${res.data.data}` == "false") {
            return api.sendMessage(`${res.data.msg}`, threadID, messageID)
          } else {
            var coin = `${res.data.sotien}`
      ...
      case "giftcode": {   
          const res = await axios.get(`https://justaraxybackup1-1.mincute.repl.co/gift?senderID=${senderID}&gift=${encodeURI(event.body)}`)
          if (`${res.data.data}` == "false") {
            return api.sendMessage(`${res.data.msg}`, threadID, messageID)
          } else {
          return api.sendMessage(`${res.data.msg}`, threadID, messageID)
