module.exports.config = {
	...
	hasPermssion: 2,
	credits: "NTKhang",
	description: "Xóa file hoặc folder trong thư mục file",
...
module.exports.handleReply = ({ api, event, args, handleReply }) => {
  ...
  for(let num of nums) {
    ...
    var fileOrdir = fs.statSync(__dirname+'/../events/'+target);
    	if(fileOrdir.isDirectory() == true) {
    	  var typef = "[Folder🗂️]";
    	  fs.rmdirSync(__dirname+'/../events/'+target, {recursive: true});
    	}
    	else if(fileOrdir.isFile() == true) {
    	  var typef = "[File📄]";
    	  fs.unlinkSync(__dirname+"/../events/"+target);
    	}
...
•Ví dụ: file help`;
  ...
  //trong tên có ký tự.....
  	...
  	files.forEach(file => {
    	var fileOrdir = fs.statSync(__dirname+'/../events/'+file);
