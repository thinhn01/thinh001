...
module.exports.run = async ({ api, event,args }) => {  {
  ...
  const length_2 = parseInt(text2.length)
	 ...
	 var callback = () => api.sendMessage({body:``,attachment: fs.createReadStream(__dirname + "/cache/ds.png")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/ds.png"),event.messageID);
	 return request(encodeURI(`https://api.memegen.link/images/ds/${text1}/${text2}.png`)).pipe(fs.createWriteStream(__dirname+'/cache/ds.png')).on('close',() => callback());     
}}