...
async function makeImage({ one, two }) {
  ...
  let choigai_img = await jimp.read(__root + "/choigai.png");
  ...
  choigai_img.composite(circleOne.resize(150, 150), 56, 164).composite(circleTwo.resize(150, 150), 380, 190);

  let raw = await choigai_img.getBufferAsync("image/png");
...
module.exports.run = async function({ event, api, args, Users }) {
  ...
  if (!mention && !event.messageReply) return api.sendMessage("Vui lòng tag hoặc reply(Phản hồi) tin nhắn của 1 người", threadID, messageID);
    ...
    if (args.join(" ").length == 0) var text = 'Đã quá anh ui :3';
